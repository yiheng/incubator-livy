...
public class TencentThriftSessionTest {
    ...
    public static void setUp() throws Exception {
        ...
        conf.put("spark.app.name", TencentThriftSessionTest.class.getName());
    ...
    public void testThriftSession() throws Exception {
        ...
        waitFor(newStreamSqlJob(s1, st3, "SELECT * FROM test", 1));

        ResultSet rs = waitFor(new FetchResultJob(s1, st3, 1));
        ColumnBuffer[] cols = rs.getColumns();

        assertEquals(Integer.valueOf(1), cols[0].get(0));
        assertEquals("one", cols[1].get(0));

        rs = waitFor(new FetchResultJob(s1, st3, 1));
        cols = rs.getColumns();
        ...
        waitFor(newStreamSqlJob(s1, st4, "SELECT * FROM test", Integer.MAX_VALUE));

        rs = waitFor(new FetchResultJob(s1, st4, Integer.MAX_VALUE));
        cols = rs.getColumns();
    ...
    private SqlJob newSqlJob(String session,
                             String stId,
                             String statement) {
        return new SqlJob(
                session,
    ...
    private SqlJob newStreamSqlJob(String session,
                                   String stId,
                                   String statement,
                                   Integer batchSize) {
        return new SqlJob(
